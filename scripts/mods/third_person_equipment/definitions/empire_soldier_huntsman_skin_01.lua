-- config file for Kruber Huntsman's Wulfhart's Array Skin

local tisch = {["units/beings/player/empire_soldier_huntsman_skin_01/third_person_base/chr_third_person_mesh"] = {
  healthkit_first_aid_kit_01 = {
    left = {
      angle = {
        -1.5709999799728,
        -3.1419999599457,
        -2.6229999065399
      },
      offset = {
        -0.030999999493361,
        0.1879999935627,
        0.087999999523163
      },
      attachement_nodes = {
        {
          source = "j_spine",
          target = 0
        }
      }
    }
  },
  wpn_side_objective_tome_01 = {
    left = {
      angle = {
        1.5049999952316,
        0.32699999213219,
        0.16200000047684
      },
      offset = {
        -0.041999999433756,
        0.19799999892712,
        0.057000000029802
      },
      attachement_nodes = {
        {
          source = "j_spine",
          target = 0
        }
      }
    }
  },
  es_repeating_handgun = {
    right = {
      angle = {
        -3.0190000534058,
        -0.041000001132488,
        1.1679999828339
      },
      offset = {
        -0.0060000000521541,
        0.25999999046326,
        0.28200000524521
      },
      scale = 1
    }
  },
  es_1h_sword = {
    right = {
      angle = {
        0,
        0.3120000064373,
        0
      },
      offset = {
        0.023000000044703,
        0.089000001549721,
        -0.065999999642372
      },
      scale = 1
    }
  },
  es_bastard_sword = {
    right = {
      offset = {
        0,
        -0.0060000000521541,
        0.15999999642372
      },
      angle = {
        0.041000001132488,
        0,
        -0.03999999910593
      },
      scale = 1
    }
  },
  es_2h_war_hammer = {
    right = {
      offset = {
        0,
        0,
        -0.08500000089407
      },
      angle = {
        0,
        0,
        0
      },
      scale = 1
    }
  },
  ww_longbow = {
    left = {
      offset = {
        0.0040000001899898,
        -0.018999999389052,
        0.032000001519918
      },
      angle = {
        0.043999999761581,
        0.16099999845028,
        0.13699999451637
      },
      scale = 1
    }
  },
  grenade_frag_02 = {
    right = {
      angle = {
        1.6319999694824,
        -0.40799999237061,
        0.2419999986887
      },
      offset = {
        0.10999999940395,
        0.19499999284744,
        -0.082999996840954
      },
      attachement_nodes = {
        {
          source = "j_spine",
          target = 0
        }
      }
    },
    left = {
      angle = {
        0.77499997615814,
        0.1630000025034,
        -0.32199999690056
      },
      offset = {
        0.20100000500679,
        0.16200000047684,
        -0.096000000834465
      },
      attachement_nodes = {
        {
          source = "j_spine",
          target = 0
        }
      }
    }
  },
  es_1h_sword_shield = {
    right = {
      angle = {
        0,
        0.3120000064373,
        0
      },
      offset = {
        0.023000000044703,
        0.089000001549721,
        -0.065999999642372
      },
      scale = 1
    },
    left = {
      offset = {
        0.032000001519918,
        0.10999999940395,
        0.026000000536442
      },
      angle = {
        -0.12200000137091,
        -0.1630000025034,
        0.03999999910593
      },
      scale = 1
    }
  },
  es_handgun = {
    right = {
      angle = {
        -3.0190000534058,
        -0.041000001132488,
        1.1679999828339
      },
      offset = {
        -0.0060000000521541,
        0.25999999046326,
        0.28200000524521
      },
      scale = 1
    }
  },
  potion_damage_boost_01 = {
    offset = {
      0.039000000804663,
      -0.064999997615814,
      0.19900000095367
    },
    angle = {
      1.7539999485016,
      0.1630000025034,
      -0.03999999910593
    },
    attachement_nodes = {
      {
        source = "j_spine",
        target = 0
      }
    }
  },
  es_1h_mace = {
    right = {
      offset = {
        0.010999999940395,
        0.083999998867512,
        -0.043000001460314
      },
      angle = {
        0,
        0.16099999845028,
        0.18199999630451
      },
      scale = 1
    }
  },
  potion_speed_boost_01 = {
    angle = {
      1.99899995327,
      0.19799999892712,
      -0.19799999892712
    },
    offset = {
      0.039000000804663,
      -0.064999997615814,
      0.20499999821186
    },
    attachement_nodes = {
      {
        source = "j_spine",
        target = 0
      }
    }
  },
  potion_cooldown_reduction_01 = {
    angle = {
      1.99899995327,
      0.19799999892712,
      -0.19799999892712
    },
    offset = {
      0.039000000804663,
      -0.064999997615814,
      0.20499999821186
    },
    attachement_nodes = {
      {
        source = "j_spine",
        target = 0
      }
    }
  },
  es_dual_wield_hammer_sword = {
    right = {
      scale = 1,
      offset = {
        0.19499999284744,
        0.12300000339746,
        -0.18600000441074
      },
      angle = {
        -1.6430000066757,
        -0.19300000369549,
        0.483000010252
      },
      attachement_nodes = {
        {
          source = "j_spine",
          target = 0
        }
      }
    },
    left = {
      scale = 1,
      offset = {
        0.14900000393391,
        0.12300000339746,
        0.20800000429153
      },
      angle = {
        -1.6430000066757,
        0.14499999582767,
        0.60399997234344
      },
      attachement_nodes = {
        {
          source = "j_spine",
          target = 0
        }
      }
    }
  },
  es_deus_01 = {
    right = {
      scale = 0.93,
      offset = {
        0.0099999997764826,
        -0.17700000107288,
        0
      },
      angle = {
        1.8980000019073,
        0,
        0
      },
      attachement_nodes = {
        {
          source = "j_spine",
          target = 0
        }
      }
    },
    left = {
      offset = {
        0.032000001519918,
        0.10999999940395,
        0.026000000536442
      },
      angle = {
        -0.12200000137091,
        -0.1630000025034,
        0.03999999910593
      },
      scale = 1
    }
  },
  es_2h_sword_executioner = {
    right = {
      angle = {
        0.041000001132488,
        0,
        0.03999999910593
      },
      offset = {
        0,
        0,
        0
      },
      scale = 1
    }
  },
  es_1h_mace_shield = {
    right = {
      offset = {
        0.010999999940395,
        0.083999998867512,
        -0.043000001460314
      },
      angle = {
        0,
        0.16099999845028,
        0.18199999630451
      },
      scale = 1
    },
    left = {
      offset = {
        0.032000001519918,
        0.10999999940395,
        0.026000000536442
      },
      angle = {
        -0.12200000137091,
        -0.1630000025034,
        0.03999999910593
      },
      scale = 1
    }
  },
  es_2h_sword = {
    right = {
      offset = {
        0,
        0,
        0
      },
      angle = {
        0.041000001132488,
        0,
        0.081000000238419
      },
      scale = 1
    }
  },
  grenade_fire_01 = {
    right = {
      angle = {
        1.6319999694824,
        -0.40799999237061,
        0.2419999986887
      },
      offset = {
        0.10999999940395,
        0.19499999284744,
        -0.1089999973774
      },
      attachement_nodes = {
        {
          source = "j_spine",
          target = 0
        }
      }
    },
    left = {
      angle = {
        1.5909999608994,
        -0.52999997138977,
        -0.081000000238419
      },
      offset = {
        0.083999998867512,
        0.14900000393391,
        -0.1410000026226
      },
      attachement_nodes = {
        {
          source = "j_spine",
          target = 0
        }
      }
    }
  },
  es_2h_halberd = {
    right = {
      angle = {
        0,
        0,
        0
      },
      offset = {
        0,
        0,
        0
      },
      scale = 1
    }
  },
  grenade_frag_01 = {
    right = {
      angle = {
        1.6319999694824,
        -0.40799999237061,
        0.2419999986887
      },
      offset = {
        0.10999999940395,
        0.19499999284744,
        -0.082999996840954
      },
      attachement_nodes = {
        {
          source = "j_spine",
          target = 0
        }
      }
    },
    left = {
      angle = {
        0.77499997615814,
        0.1630000025034,
        -0.32199999690056
      },
      offset = {
        0.20100000500679,
        0.16200000047684,
        -0.096000000834465
      },
      attachement_nodes = {
        {
          source = "j_spine",
          target = 0
        }
      }
    }
  },
  es_2h_heavy_spear = {
    right = {
      offset = {
        0,
        0,
        0
      },
      angle = {
        0,
        0,
        0
      },
      scale = 1
    }
  },
  potion_healing_draught_01 = {
    left = {
      angle = {
        1.7669999599457,
        -0.2039999961853,
        -0.12099999934435
      },
      offset = {
        0.018999999389052,
        -0.14599999785423,
        0.10800000280142
      },
      attachement_nodes = {
        {
          source = "j_spine",
          target = 0
        }
      }
    }
  },
  es_blunderbuss = {
    right = {
      angle = {
        -3.0190000534058,
        -0.041000001132488,
        1.1679999828339
      },
      offset = {
        -0.0060000000521541,
        0.25999999046326,
        0.28200000524521
      },
      scale = 1
    }
  },
  grenade_fire_02 = {
    right = {
      angle = {
        1.6319999694824,
        -0.40799999237061,
        0.2419999986887
      },
      offset = {
        0.10999999940395,
        0.19499999284744,
        -0.082999996840954
      },
      attachement_nodes = {
        {
          source = "j_spine",
          target = 0
        }
      }
    },
    left = {
      angle = {
        0.77499997615814,
        0.1630000025034,
        -0.32199999690056
      },
      offset = {
        0.20100000500679,
        0.16200000047684,
        -0.096000000834465
      },
      attachement_nodes = {
        {
          source = "j_spine",
          target = 0
        }
      }
    }
  },
  wpn_grimoire_01 = {
    angle = {
      1.5709999799728,
      -1.8980000019073,
      0.097000002861023
    },
    offset = {
      -0.052000001072884,
      -0.052000001072884,
      -0.23199999332428
    },
    attachement_nodes = {
      {
        source = "j_spine",
        target = 0
      }
    }
  }
}
}

return tisch